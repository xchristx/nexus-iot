...
(sus entradas y salidas, y las reglas de su modo automático), parecido a los
...
entre envíos), salidas incómodas de manejar y ninguna vista central para el docente.
...
2. Arranca sin nada configurado (salvo que el docente le haya cargado un kit al
   curso), y en **Configurar** declara, cambia o borra sus entradas, salidas y
   reglas.
...
2. En "Configurar" declará tus entradas (lo que tu placa mide o lee) y tus
   salidas (lo que prende y apaga), y armá las reglas del modo automático.
3. En "Mis datos" tenés todo listo para copiar:
   - la clave de tu placa
   - la URL para leer datos desde Kodular (Web.Get, sin headers)
   - cómo prender una salida y cómo manejar el modo automático (Web.PostText)
...
POST rpc/guardar_canal   {"p_admin":"...","p_canal":{"id":"suelo","tipo":"entrada","unidad":"%"}}
POST rpc/borrar_canal    {"p_admin":"...","p_id":"suelo"}
POST rpc/guardar_regla   {"p_admin":"...","p_regla":{"salida":"bomba","entrada":"suelo","condicion":"<","umbral":35,"hist":2}}
POST rpc/borrar_regla    {"p_admin":"...","p_salida":"bomba"}
```
...
POST rpc/ajustar_regla   {"p_clave":"...","p_salida":"bomba","p_cambios":{"activa":true,"umbral":30}}
...
todavía no recogió: mientras una salida aparezca ahí, la app debería mostrar
"enviando…".
...
Un valor como texto se rechaza. Una entrada declarada que no llega, o algo que llega
...
era el estado de antes; sin este atajo, la app vería la salida con el valor viejo durante un
...
placa va dentro del APK y es extraíble, así que solo puede leer, prender salidas y
ajustar reglas. Crear o borrar entradas, salidas y reglas exige entrar al portal
con el PIN.

**Las reglas corren en la placa, acotadas.** Así la placa sigue regulando sin
internet. Para que la IA las implemente bien: una sola por salida, solo `>` o `<`,
...
Todo es de tablas: entradas, salidas y reglas. Agregar una entrada o una salida es
agregar una fila. Las tablas traen un kit de EJEMPLO (DHT22 + bomba, ventilador y
LED), el mismo de `PROMPT.md`: cada alumno las cambia por lo que declaró. Las
reglas se guardan en la flash para seguir regulando al arrancar sin red, y el WiFi
nunca bloquea el loop: si se corta, se siguen leyendo entradas y evaluando reglas.

Si tocás `src/main.cpp`, regenerá el `.ino` y el zip que se reparte:

```bash
# 1. el sketch de Arduino IDE (desde la raíz del repo). Las dos últimas
#    expresiones vacían el WiFi, por si src/main.cpp tiene cargado el tuyo.
...
La plantilla por defecto del curso (en `backend/01-esquema.sql`) está vacía. El kit
de ejemplo vive en tres lugares que conviene mantener iguales: la receta "Cargar un
kit" de `backend/03-curso-ejemplo.sql`, `portal/src/plantilla-ejemplo.js` (de donde
